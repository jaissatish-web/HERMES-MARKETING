...
const services = JSON.parse(localStorage.getItem('gateHubServices') || 'null') || [...defaultServices];
...
const approvals = [
  ...
  { title: 'Enable automated Social Publishing', detail: 'Current recommendation: keep social publishing in approval-required mode until the publishing workflow is validated.', risk: 'Medium' },
...
function persistServices() {
  localStorage.setItem('gateHubServices', JSON.stringify(services));
}
...
function renderServices() {
  el('serviceSummary').innerHTML = services.slice(0, 5).map(s => `<div class="service-mini"><div><strong>${escapeHtml(s.name)}</strong><p>${escapeHtml(s.purpose)}</p></div><span class="badge ${s.status === 'Online' ? 'green' : 'amber'}">${escapeHtml(s.status)}</span></div>`).join('');
  el('serviceTable').innerHTML = services.map((s, index) => `<tr><td><strong>${escapeHtml(s.name)}</strong></td><td>${escapeHtml(s.purpose)}</td><td>${escapeHtml(s.provider)}</td><td>${escapeHtml(s.model)}</td><td><span class="badge ${s.status === 'Online' ? 'green' : 'amber'}">${escapeHtml(s.status)}</span></td><td>${escapeHtml(s.mode)}</td><td><button class="text-button" data-service="${index}">Manage</button></td></tr>`).join('');
  document.querySelectorAll('[data-service]').forEach(b => b.addEventListener('click', () => openService(Number(b.dataset.service))));
}

function renderProviders() {
  el('providerCards').innerHTML = providers.map(p => `<div class="service-card"><div class="panel-title"><h3>${escapeHtml(p.name)}</h3><span class="badge ${p.status === 'Connected' ? 'green' : 'amber'}">${escapeHtml(p.status)}</span></div><p>${escapeHtml(p.purpose)}</p><div class="muted">Models: ${p.models.map(escapeHtml).join(', ')}</div><div style="margin-top:14px"><div class="panel-title"><span class="small">Configured usage</span><strong>${escapeHtml(p.usage)}</strong></div><div class="meter"><span style="width:${parseInt(p.usage, 10) || 0}%"></span></div></div><div class="form-actions" style="margin-top:15px"><button class="secondary">Test</button><button class="primary">Manage</button></div></div>`).join('');
}

function renderPermissions() {
  el('permissionTable').innerHTML = `<div class="permission-row permission-head"><span>Service</span><span>Read</span><span>Draft</span><span>Execute</span><span>Publish</span></div>` + permissions.map(row => `<div class="permission-row"><strong>${escapeHtml(row[0])}</strong><span>✓ ${escapeHtml(row[1])}</span><span>✓ ${escapeHtml(row[2])}</span><span>✓ ${escapeHtml(row[3])}</span><span>○ ${escapeHtml(row[4])}</span></div>`).join('');
}

function renderApprovals() {
  el('approvalList').innerHTML = approvals.map((a, i) => `<div class="approval"><div><span class="eyebrow">Approval #${i + 1} · ${escapeHtml(a.risk)} risk</span><h3>${escapeHtml(a.title)}</h3><p>${escapeHtml(a.detail)}</p></div><div class="approval-actions"><button class="secondary" data-reject="${i}">Reject</button><button class="primary" data-approve="${i}">Review</button></div></div>`).join('');
...
function renderBudgets() {
  el('budgetCards').innerHTML = budgets.map(b => { const pct = b.limit ? Math.round(b.used / b.limit * 100) : 0; return `<div class="service-card"><div class="panel-title"><h3>${escapeHtml(b.name)}</h3><strong>${escapeHtml(b.unit)}${escapeHtml(b.used)}</strong></div><p>Limit ${escapeHtml(b.unit)}${escapeHtml(b.limit)} / month</p><div class="meter"><span style="width:${pct}%"></span></div><p>${pct}% used</p></div>`; }).join('');
}

function renderHealth() {
  el('healthCards').innerHTML = health.map(h => `<div class="service-card"><div class="panel-title"><h3>${escapeHtml(h.name)}</h3><span class="badge ${h.status === 'Healthy' ? 'green' : 'amber'}">${escapeHtml(h.status)}</span></div><p>${escapeHtml(h.detail)}</p></div>`).join('');
}

function renderActivity() {
  el('activityList').innerHTML = activity.map(a => `<div class="activity-row"><span>${escapeHtml(a[0])}</span><strong>${escapeHtml(a[1])}</strong><span>${escapeHtml(a[2])}</span><span class="badge ${a[3] === 'success' ? 'green' : 'amber'}">${escapeHtml(a[3])}</span></div>`).join('');
}

function openService(index) {
  const service = services[index];
  if (!service) return;
  alert(`Prototype: managing “${service.name}”. Production UI will provide provider, model, credential reference, purpose, permissions, budget, approval mode, health and logs.`);
}
...
el('pauseAll').addEventListener('click', () => alert('Prototype emergency control: production version will require server-side permission and immediately disable automated external actions.'));
