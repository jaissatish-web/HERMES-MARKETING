  ...
  return (
    <main className="shell">
      <aside className="sidebar">
        ...
        <nav>{sections.map((item, i) => <button key={item} className={i === 0 ? 'active' : ''} onClick={() => item === 'Providers & Models' ? router.push('/dashboard/providers') : item === 'Credentials' ? router.push('/dashboard/credentials') : undefined}>{item}</button>)}</nav>
      ...
      <section className="content">
        ...
        <section className="panel"><div className="panelHead"><div><h2>Founder control</h2><p>Your authenticated GATE HUB workspace is connected to the real Supabase database.</p></div></div><div className="grid"><article><div className="icon">S</div><div className="serviceText"><h3>Services</h3><p>Manage capability slots and activation.</p></div><span className="badge">Connected</span></article><article onClick={() => router.push('/dashboard/providers')} style={{ cursor: 'pointer' }}><div className="icon">P</div><div className="serviceText"><h3>Providers & Models</h3><p>Manage the companies that supply your AI/API capabilities.</p></div><span className="badge">Provider Manager</span></article><article onClick={() => router.push('/dashboard/credentials')} style={{ cursor: 'pointer' }}><div className="icon">🔐</div><div className="serviceText"><h3>Credential Vault</h3><p>Connect API keys securely without exposing them to the browser.</p></div><span className="badge">Secure</span></article></div></section>
      </section>
    </main>
  );
}
